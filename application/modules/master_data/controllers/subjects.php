...
class Subjects extends MX_Controller {
	...
	public function index(){
		...
		$data['head']   = 'Subjects';
		$data['icon']	= '<span class="iconfa-book"></span>';
		$data['country']=$this->db->query("select * from country order by country_name")->result();
		$data['org']	= $this->db->query("select a.*,b.region_name,c.country_name from city a join region b on a.region_code = b.region_code join country c on b.country_code = c.country_code order by a.city_name")->result();
		$this->page->view('lesson_time_index',$data);	
	}
	...
	function check_code($code){
		...
		echo $this->db->get('city')->num_rows();
	}
