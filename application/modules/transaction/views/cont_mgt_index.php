...
function light($id){
	if($id==1){
		#return '<span style="background:#59BA00 !important;padding:5px 12px">&nbsp;</span>';
		return ' <span class="label label-success">Yes</span>';
	}else{
		return ' <span class="label label-important" style="background:#CC0000">No</span>';
	}
}
function get_curr_user($id){
	$CI =& get_instance();
	return $CI->db->query("select * from user_login where org_id = '$id'")->num_rows();
}
?>	
               ...
               <thead>
				...
				<tbody id="trow">
					...
					<tr style="font-size:9pt;background:white">
						...
						<td><?php echo get_curr_user($row->org_code);?></td>
						<td><?php echo $row->max_mess;?></td>
						<td>0</td>
						<td><?php echo $row->max_mess_mt;?></td>
						<td>0</td>
						<td><?php echo light($row->ads);?></td>
						<td><?php echo light($row->feat_add_score);?></td>
						<td><?php echo light($row->feat_add_poll);?></td>
						<td><?php echo light($row->feat_add_sfee);?></td>
						<td>
